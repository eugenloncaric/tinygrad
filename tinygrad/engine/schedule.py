...
from tinygrad.ops import UOp, Ops, GroupOp, Variable, PatternMatcher, UPat, type_verify, graph_rewrite_map, graph_rewrite, track_rewrites
from tinygrad.ops import merge_views, symbolic_simple, view_left
from tinygrad.device import Buffer
from tinygrad.helpers import Metadata, all_int, unwrap
...
remove_movement_ops = merge_views+PatternMatcher([
  (UPat(Ops.VIEW, name="view", src=(UPat.var("x"),)), lambda view,x: x if view.st.contiguous and x.shape == view.shape else None),
])
...
def add_buffer(ctx:dict[UOp, UOp], root:UOp):
  if root.st is None or root.base.op is Ops.BUFFER: return None
